...
docker run --rm --name graal -v $(pwd):/working oracle/graalvm-ce:1.0.0-rc10 \
    /bin/bash -c "native-image --enable-url-protocols=http \
                    ...
                    -H:+ReportUnsupportedElementsAtRuntime --no-server -jar /working/build/libs/kotlin-graalvm-custom-runtime-demo-1.0.jar \
                    ; \
                    cp kotlin-graalvm-custom-runtime-demo-1.0 /working/build/server"

rm -rf build/package
mkdir build/package
mkdir build/package/server

cp -r build/server build/package
